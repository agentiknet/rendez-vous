...
import type { Room, Tier, Member } from "../rooms/types.ts"
...
export function createHttpServer(service: RoomService, mediaHooks?: HttpMediaHooks, stateHooks?: HttpStateHooks): Server {
  ...
  const mcpPersonal = createMcpPersonalHandler({
    ...
    // BRIEF-12: the panel's listening half. Drain is the ONE `outboxFor`;
    // ack is the ONE cursor writer; the room-web member is resolved or
    // created server-side, keyed by the principal — so the panel still holds
    // no token, no claim and no room CODE.
    roomRead: {
