...
function capabilityLines(code: string, opts?: { appDir: string }): string[] {
  const lines = [
    ...
    "ANSWER THE PERSON WHO SPOKE. That is the default: `say` with `to` set to just their `member_id`. Two people asking unrelated things must not each receive both answers on their phone.",
