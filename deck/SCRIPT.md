...
## 1:50 — Slide 6: The silent-failure class

"This is the strongest thing we found. Four independent bugs, one shape.
Omit queue: true and a mid-turn message just vanishes. Resume a session
and the fan-out cursor freezes, so every reply after that resume vanishes
too. Pin an inbound route to one account and it silently never fires.
Ask the agent for Gmail access it already effectively has, and it
confidently proposes rebuilding it from scratch. None of these crash. All
four look like working code or correct configuration. This is evidence you
only get by running it."

## 2:25 — Slide 7: Credentials per-member
...
## 2:45 — Slide 8: Second brain is a parameter
...
## 3:05 — Slide 9: Built on our own runtime
...
in twenty to sixty minutes even when our own records still call them
paused. We're sharing an existing workspace's Telegram routing, so an
existing catch-all route can fire alongside ours on the same message —
