...
test("BRIEF-13 step 4 bugfix: 'leave <slug>' actually leaves the NAMED room, not whichever room the sender happens to be in", async () => {
  // Before this fix, parseCommand parsed the argument but handleLeave
  // discarded it and always called handleCommand with a bare {kind: "leave"}
  // — so "leave <slug>" behaved exactly like bare "leave", silently ignoring
  // the room the sender actually named.
  ...
  const left = await service.handleInbound(alice(`leave ${created.room.slug}`))
  assert.equal(left.kind, "left")
  if (left.kind !== "left") return
  assert.equal(left.room.code, created.room.code, "the room actually left must be the one named")
  assert.equal(store.get(created.room.code)?.members.length, 0)
})

test("BRIEF-13 step 4 bugfix: 'leave <slug>' on a room the sender is ambiguously in touches ONLY the named room, never every room", async () => {
  const { service, store } = await buildHarness()
  ...
  // ensureMembership's move semantics — the exact "ambiguous" shape R1
  // documents as a broken invariant that can still exist in the store.
  const roomB = await store.create()
  await store.addMember(roomB.code, { displayName: "Alice", tier: "messenger", address: alice("").address })
  assert.equal(store.findByAddress(alice("").address).kind, "ambiguous", "the fixture must start genuinely ambiguous")

  const left = await service.handleInbound(alice(`leave ${roomA.room.slug}`))
  assert.equal(left.kind, "left")
  if (left.kind !== "left") return
  assert.equal(left.room.code, roomA.room.code, "only the NAMED room is left")

  assert.equal(store.get(roomA.room.code)?.members.length, 0, "the named room lost its member")
  assert.equal(store.get(roomB.code)?.members.length, 1, "the room NOT named must be untouched, even though the address was ambiguous")
})

test("'leave <code-or-slug>' naming a room the sender does not belong to is refused as not-in-room, and a room that does not exist as unknown-code", async () => {
  const { service, store } = await buildHarness()
  ...
  await store.addMember(stranger.code, { displayName: "Bob", tier: "messenger", address: bob("").address })

  const notMember = await service.handleInbound(alice(`leave ${stranger.slug}`))
  assert.deepEqual(notMember, { kind: "not-in-room" })
  assert.equal(store.get(created.room.code)?.members.length, 1, "a refused leave must not touch the room the sender actually IS in")

  // SLUG_PATTERN requires exactly three hyphenated words; this one is
  // shaped like a real slug but was never generated by any store.create().
  const unknown = await service.handleInbound(alice("leave harbor-lantern-ghost"))
  assert.deepEqual(unknown, { kind: "unknown-code" })
})
